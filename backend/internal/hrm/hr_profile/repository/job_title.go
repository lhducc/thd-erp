...
func (r *jobTitleStore) GetJobTitleById(ctx context.Context, id string) (*model.JobTitle, error) {
	var jobTitle model.JobTitle
	if err := r.db.WithContext(ctx).Table("jobtitle").
...
func (r *jobTitleStore) GetAllJobTitle(ctx context.Context) ([]model.JobTitle, error) {

	var positions []model.JobTitle
	if err := r.db.WithContext(ctx).Table("jobtitle").Find(&positions).Error; err != nil {
		return nil, err
	}
...
func (r *jobTitleStore) UpdateJobTitleById(ctx context.Context, id string, data *model.JobTitleCreate) error {
	return r.db.WithContext(ctx).Table("jobtitle").
		Where("job_title_id = ?", id).
		Updates(data).Error
}

func (r *jobTitleStore) DeleteJobTitleById(ctx context.Context, id string) error {
	return r.db.WithContext(ctx).Table("jobtitle").
		Where("job_title_id = ?", id).
		Delete(nil).Error
}
