...
export const exportEmployeeExcelApi = async (): Promise<File> => {
  const response = await api.get(
    "employee/export?fields=employee_id,fullname,email,manager,birthday",
    {
      responseType: "blob",
    }
  );
...
export const getEmployeeByRoleNameApi = async (
  roleName: string
): Promise<EmployeeNameAndRole[]> => {
...
export const getEmployeesByManagerApi = async (): Promise<
  ManagerEmployee[]
> => {
...
export const getAllEmployeesActiveApi = async (
  page: number = 1,
  pageSize: number = 9999
) => {
  const response = await api.get(
    `/employee/active?page=${page}&pageSize=${pageSize}`
  );
  if (response.data && response.data.data) {
    const list = response.data.data.data;
    const result: Employee[] = [];
