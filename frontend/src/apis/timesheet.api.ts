...
import type {TimesheetInfor, TimesheetList, TimesheetResponse} from "@/types/timesheet.ts";
...
export const getEmployeeTimesheetApi = async (month: number, year: number): Promise<TimesheetList[]> => {
    const response = await api.get(`/timesheet/personal?month=${month}&year=${year}`);
    if (response.data.data && response.data.data.length > 0) {
        const result = await api.get(`/timesheet/personal?month=${month}&year=${year}`);
    }
    return response.data.data;
}
