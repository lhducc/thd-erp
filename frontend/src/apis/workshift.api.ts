...
export const getAllWorkshiftApi = async (): Promise<WorkShift[]> => {
    try {
        ...
        // Return only non-deleted work shifts
        return mockWorkShifts.filter(shift => !shift.is_deleted);
    } catch (error) {
...
export const createWorkshiftApi = async (payload: WorkShiftRequest): Promise<WorkShift> => {
    try {
        // Simulate API delay
        await new Promise(resolve => setTimeout(resolve, 500));
        ...
        const newWorkShift: WorkShift = {
            ...payload,
            workshift_id: (mockWorkShifts.length + 1).toString(),
            created_date: new Date().toISOString(),
            is_deleted: false,
            creator: {
                employee_id: payload.created_by
            }
        };
...
export const updateWorkshiftApi = async (id: string, payload: WorkShiftRequest): Promise<WorkShift> => {
    try {
        ...
        const index = mockWorkShifts.findIndex(shift => shift.workshift_id === id);

        if (index === -1) {
            throw new Error("Workshift not found");
        }

        const updatedWorkShift: WorkShift = {
            ...mockWorkShifts[index],
            ...payload,
            creator: {
                employee_id: payload.created_by
            }
        ...
        mockWorkShifts[index] = updatedWorkShift;
...
export const deleteWorkshiftApi = async (id: string): Promise<void> => {
    try {
        ...
        mockWorkShifts[index].is_deleted = true;
    } catch (error) {
