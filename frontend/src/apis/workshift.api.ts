...
} from "@/types/employee-workshift.ts";
...
export const createWorkshiftApi = async (
  payload: Omit<WorkShiftRequest, "workshift_id">
): Promise<Workshift> => {
  const response = await api.post("/workshifts", payload);
  return response.data.data;
};

export const updateWorkshiftApi = async (
  id: string,
...
export const deleteEmployeeWorkshiftByEmployeeApi = async (
  id: number
): Promise<void> => {
  try {
    await api.delete(`/employee-workshifts/personal/${id}`);
  } catch (error) {
...
export const getEmployeeWorkshiftsApi = async (
  month: number,
  year: number
): Promise<EmployeeWorkshift[]> => {
  try {
    const response = await api.get(
      `/employee-workshifts/personal?month=${month}&year=${year}`
    );
    return response.data.data;
  } catch (error) {
...
export const getAllEmployeeWorkshiftsApi = async (): Promise<
  EmployeeWorkshift[]
> => {
  try {
...
export const updateEmployeeWorkshiftApi = async (
  ...
  try {
    const response = await api.put(
      `/employee-workshifts/${employeeWorkshiftId}`,
      {
...
export const getEmployeeWorkshiftsByManagerApi = async (
  targetDate?: string
): Promise<ManagerEmployeeSchedule[]> => {
  try {
    const params = targetDate ? { targetDate } : {};
    const response = await api.get("/employee-workshifts/by-manager", {
      params,
    });
    return response.data.data;
  } catch (error) {
    console.error("Error fetching employee workshifts by manager:", error);
    throw error;
  }
};
