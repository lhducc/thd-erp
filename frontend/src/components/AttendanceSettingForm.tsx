...
} from "@/apis/attendance-management.api.ts";
...
const formSchema = z.object({
  attendance_category_name: z
    .string()
    .min(1, "Tên hình thức chấm công là bắt buộc"),
  office_id: z.string(),
  is_camera: z.boolean(),
  is_gps: z.boolean(),
...
const AttendanceSettingForm = ({ editBtn, data, type, refetch }: Props) => {
  ...
  const {
    mutateAsync: createAttendanceSetting,
    isPending: pendingCreateAttendanceSetting,
  } = useMutation({
  ...
  const {
    mutateAsync: updateAttendanceSetting,
    isPending: pendingUpdateAttendanceSetting,
  } = useMutation({
    ...
    try {
      ...
      if (type === "edit" && data) {
        await updateAttendanceSetting({
          id: data.attendance_category_id || "",
          payload,
        });
      } else {
      ...
      <DialogContent className="md:max-w-2xl">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder="Nhập tên hình thức chấm công"
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
            ...
            >
              {pendingCreateAttendanceSetting ||
              pendingUpdateAttendanceSetting ? (
