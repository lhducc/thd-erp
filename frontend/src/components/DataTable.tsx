...
export default function DataTable<TData, TValue>({
  ...
  // Number of skeleton rows to show (matches current page size)
    ...
    <div className="md:max-w-[1580px] w-full]">
      <div className="flex lg:flex-row flex-col items-center py-4 gap-8">
        ...
        <InputWithIcon
          ...
          disabled={isLoading}
        />
        {isLoading ? <Skeleton className="h-[50px] w-[150px]" /> : buttonCreate}
      </div>
      {navLink}
        ...
        <Table>
          <TableHeader>
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow
                key={headerGroup.id}
                className="bg-[#DB3B21] hover:bg-[#DB3B21]"
              >
                ...
                <TableRow key={`skeleton-${index}`}>
                  {columns.map((column, colIndex) => (
                    <TableCell key={`skeleton-cell-${index}-${colIndex}`}>
                      <div className="h-4 bg-gray-300 rounded-sm dark:bg-gray-700 max-w-[640px] mb-2.5 mx-auto animate-pulse"></div>
                    </TableCell>
                  ))}
                </TableRow>
              ))
