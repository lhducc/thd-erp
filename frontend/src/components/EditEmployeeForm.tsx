...
const EditEmployeeForm = ({ open, setOpen, data, refetchEmployee }: Props) => {
  ...
  const [workScheduleType, setWorkScheduleType] = useState<boolean>(
    workschedulesAuto?.includes(data.schedule_id) || false
  ); // True là tự động, False là đăng ký
  function toYMD(date: string | Date): string {
    return new Date(date).toISOString().split("T")[0];
  }

  const workSchedules = workScheduleType
    ? workschedulesAuto
    : workschedulesRegister;
  ...
  async function onSubmit(values: z.infer<typeof form>) {
    ...
    try {
      ...
      await clientQuery.invalidateQueries({
        queryKey: ["employeeId", data.employee_id],
      });
      refetchEmployee();
      setIsLoading(false);
  ...
  useEffect(() => {
    ...
    if (
      workschedulesAuto?.some(
        (s) => s.work_schedule_id === Number(data.schedule_id)
      )
    ) {
      ...
      <DialogContent className="w-[1000px] py-10 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex px-[30px] items-center relative">
            ...
            <DialogTitle className="text-3xl w-[186px] h-[54px] font-semibold flex items-center justify-center absolute right-[30px] rounded-3xl border-2 border-gray-400">
              {data.employee_id}
            </DialogTitle>
          </div>
        </DialogHeader>
                    ...
                    render={({ field }) => (
                      <FormItem className="mt-[20px]">
                        ...
                        <FormControl>
                          ...
                          >
                            <option value="">
                              Chọn lịch làm việc <Asterisk />
                            </option>
                            {workSchedules?.map((item) => {
                              return (
                                <option value={item.work_schedule_id}>
