...
import {
  deleteAttendanceSettingApi,
  getAllAttendanceManagementAPI,
...
const AttendanceManagementPage = () => {
  ...
  const columns: ColumnDef<AttendanceSetting>[] = [
    ...
    },
    {
      ...
      header: "Định vị GPS",
      cell: ({ row }) => (
        <input type="checkbox" checked={row.original.is_gps} readOnly />
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <input type="checkbox" checked={row.original.is_camera} readOnly />
      ),
    },
    {
      accessorKey: "status",
            ...
            <ConfirmDelete
              deleteFn={() =>
                deleteAttendanceSetting(
                  attendanceSetting.attendance_category_id
                )
              }
            />
          </div>
