    ...
    return (
        <div key={employee.employee_id} className="flex">
            ...
            {monthDays.map((day, dayIndex) => {
                ...
                const assignedShifts = employeeShifts[`${employee.employee_id}-${dayIndex}`] || [];
                const actualShifts = getActualEmployeeShift(employee.employee_id, dayIndex);

                // Kiểm tra xem có ca thật và ca dự đoán trùng nhau không
                const hasOverlap = actualShifts.length > 0 && scheduledShift &&
                    actualShifts.some(actualShift => isShiftOverlap(actualShift, scheduledShift));
...
const Rota = () => {
    ...
    const handleDragEnd = useCallback((result: DropResult) => {
        ...
        if (source.droppableId.includes('-') && destination.droppableId.includes('-') && source.droppableId !== destination.droppableId) {
            ...
            setEmployeeShifts(prev => {
                ...
                if (!shiftExists && movedShift) {
                    ...
                    const insertIndex = Math.min(destination.index, newShifts[destKey].length);

                    newShifts[destKey] = [
                        ...newShifts[destKey].slice(0, insertIndex),
                        {...movedShift, tempId: `${movedShift.workshift_id}-${Date.now()}`},
        ...
        if (source.droppableId.includes('-') && destination.droppableId.includes('-') && source.droppableId === destination.droppableId) {
            ...
            setEmployeeShifts(prev => {
                ...
                // Kiểm tra index có hợp lệ không
                if (source.index >= shiftsCopy.length || destination.index > shiftsCopy.length) return newShifts;
    ...
    const isShiftOverlap = useCallback((actualShift: Workshift, scheduledShift: any): boolean => {
        ...
        return actualShift.workshift_id === scheduledShift.workshift_id ||
            (actualShift.start_time === scheduledShift.start_time &&
                actualShift.end_time === scheduledShift.end_time);
    }, []);

    const getScheduledShift = useCallback((employeeId: string, dayIndex: number) => {
    ...
    const handleAssignShifts = useCallback(() => {
        ...
        if (shiftsToRegister.length === 0) {
            toast.warning("Không có ca nào để phân. Hãy kéo ca từ danh sách bên phải vào lịch.");
            return;
        }
    ...
    const totalAssignedShifts = useMemo(() => {
        return Object.values(employeeShifts).reduce((total, shifts) => total + shifts.length, 0);
    }, [employeeShifts]);
                    ...
                    <div className="min-w-max">
                        ...
                        <div className="flex">
                            ...
                            {monthDays.map((day, i) => (
                                <div
                                    key={i}
                                    className={`p-2 text-center font-medium border sticky top-0 ${
                                        day.getDay() === 0 || day.getDay() === 6 ? 'bg-red-50' : ''
                                    } ${
