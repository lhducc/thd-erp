...
import {
  ...
  useExportTimesheet,
  useExportTimesheetCheckinCheckout,
  useGetTimesheet,
  useLockTimesheet,
  useResetTimesheet,
...
const TimesheetDetailPage = () => {
  ...
  const { mutate: updateTimesheetDetail, isPending: isUpdatingTimesheet } =
    useUpdateTimesheetDetail();
  const { mutate: lockTimesheet, isPending: isLockTimesheet } =
    useLockTimesheet();
  const { mutate: exportTimesheet } = useExportTimesheet();
  const { mutate: calculatorTimesheet, isPending: isCalculatorTimesheet } =
    useCalculatorTimesheet();
  const { mutate: resetTimesheet, isPending: isResetTimesheet } =
    useResetTimesheet();
  const [expandedRows, setExpandedRows] = useState<Set<number>>(new Set());
  const [selectedDetail, setSelectedDetail] = useState<TimesheetDetail | null>(
    null
  );
  ...
  const { mutate: exportCheckinCheckout } = useExportTimesheetCheckinCheckout();
        ...
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle className="flex justify-between items-center gap-2">
              ...
              <div className="flex gap-2">
                <Button
                  size="sm"
                  onClick={() => openEditDialog(selectedDetail)}
                >
                ...
                {selectedDetail.is_manually_adjusted && (
                  <Button
                    size="sm"
                    onClick={() =>
                      handleReset(selectedDetail.timesheet_detail_id)
                    }
          ...
          <div className="space-y-3">
            <div>
              ...
              {selectedDetail.work_shift?.workshift_name || "N/A"}
            </div>
            ...
            <div>
              ...
              {selectedDetail.is_absent ? (
                <span className="text-red-600">
                  Vắng mặt ({selectedDetail.absent_reason || "Không có lý do"})
                </span>
              ) : selectedDetail.work_days > 0 ? (
                <span className="text-green-600">Đi làm</span>
              ) : selectedDetail.leave_type ? (
                <span className="text-yellow-600">
                  Nghỉ phép ({selectedDetail.leave_type})
                </span>
              ) : (
            ...
            {selectedDetail.checkin_record && (
              <div>
                ...
                {selectedDetail.is_late && (
                  <span className="text-orange-600 ml-2">
                    (Trễ: {selectedDetail.late_minutes} phút)
                  </span>
                )}
              </div>
            )}
            ...
            <div>
              <span className="font-semibold">Số giờ làm:</span>{" "}
              {selectedDetail.work_hours} giờ
            </div>

            <div>
              <span className="font-semibold">Số công:</span>{" "}
              {selectedDetail.work_days}
            </div>
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="mb-6 p-4 bg-white rounded shadow">
        <h1 className="text-2xl font-bold mb-2">
          {timesheetData.time_sheet_list_name}
        </h1>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <p>
              <span className="font-semibold">Tháng/Năm:</span>{" "}
              {timesheetData.month}/{timesheetData.year}
            </p>
            <p>
              <span className="font-semibold">Khoảng thời gian:</span>{" "}
              {new Date(timesheetData.start_date).toLocaleDateString("vi-VN")} -{" "}
              {new Date(timesheetData.end_date).toLocaleDateString("vi-VN")}
            </p>
          </div>
          <div>
            <p>
            ...
            <p>
              ...
              {timesheetData.created_by}
            </p>
            <p>
              <span className="font-semibold">Ngày tạo:</span>{" "}
              {new Date(timesheetData.created_at).toLocaleDateString("vi-VN")}
            </p>
          </div>
          ...
          <thead>
            <tr className="bg-gray-100">
              ...
              {dayColumns?.map((col, idx) => (
                <th key={idx} className="p-1 border text-center">
                  {col.header}
                </th>
              ))}
