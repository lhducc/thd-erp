...
const TimesheetForm = ({ refresh }: { refresh: () => void }) => {
  ...
  const openButton = <Button>Tạo bảng công</Button>;
    ...
    return (
      ...
      >
        ...
        <div className={`flex gap-2 items-center w-full justify-center mt-2`}>
          ...
          <Button type="submit">
            {isPending ? <Loading /> : "Tạo bảng công"}
          </Button>
        </div>
      </form>
    );
  ...
  const columns: ColumnDef<Timesheet>[] = [
        ...
        return (
          <>
            {!row.original.is_locked ? (
              ...
              <p className="text-red-500">Đã chốt công</p>
            )}
          </>
        );
      },
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <Link
            to={`/timesheet/${row.original.timesheet_list_id}`}
