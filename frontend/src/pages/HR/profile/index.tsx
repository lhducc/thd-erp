...
const HRProfilePage = () => {
  ...
  const columns: ColumnDef<Employee>[] = [
    ...
    },
    {
      ...
      header: "Vị trí",
      cell: ({ row }: { row: any }) =>
        row.original.position?.position_name || "N/A",
    },
    {
      accessorKey: "office",
      header: "Văn phòng",
    ...
    },
    {
      ...
      cell: ({ row }: { row: any }) => {
        ...
        return (
          <button
            onClick={() =>
              changeStatus({ id: row.original.employee_id, status: nextStatus })
            }
      ...
      cell: ({ row }: { row: any }) => {
        ...
        return (
          <div className="flex gap-4">
            ...
            <ConfirmDelete
              deleteFn={() => deleteEmployee(employee.employee_id)}
            />
          </div>
        );
      },
      ...
      if (employees && employees.length > 0) {
        const options: FilterOption[] = [
          ...
          },
          {
            ...
            options: Array.from(
              new Set(
                employees.map((e) => e.job_title?.job_title).filter(Boolean)
              )
            ).map((value) => ({
              value: value as string,
              label: value as string,
          ...
          },
          {
            ...
            options: Array.from(
              new Set(
                employees.map((e) => e.position?.position_name).filter(Boolean)
              )
            ).map((value) => ({
              value: value as string,
              label: value as string,
          ...
          switch (field) {
            case "office":
              return values.includes(
                employee.department?.office?.office_name || ""
              );
            case "department":
              return values.includes(
